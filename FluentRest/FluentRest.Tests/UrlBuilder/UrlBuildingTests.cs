...
namespace FluentRest.Test.UrlBuilder
{
	...
	public class UrlBuildingTests
	{
		...
		public void uri_cannot_be_null() {
			Assert.ThrowsException<ArgumentNullException>(() => new Url((Uri)null));
		}
		...
		public void setting_query_param_array_creates_multiple() {
			...
			CollectionAssert.AreEqual(new[] { 1, 2, 3 }, q.Select(p => p.Value).ToList());
		}
		...
		[TestMethod, Ignore("tricky to do while maintaining param order. deferring until append param w/o overwriting is fully supported.")]
		...
		public void can_control_null_value_behavior_in_query_params(NullValueHandling nullValueHandling, string expected) 
		{
			var r = "http://www.mysite.com?y=2".SetQueryParams(new { x = 1, y = (string)null, z = "foo" }, nullValueHandling).ToString();
		...
		public void appending_null_path_segment_throws_arg_null_ex() {
			Assert.ThrowsException<ArgumentNullException>(() => "http://www.mysite.com".AppendPathSegment(null));
		}
		...
		public void can_parse_path_segments() {
			...
			CollectionAssert.AreEqual(new[] { "segment%201", "", "segment%203", "%3Fsegment%234" }, Url.ParsePathSegments(path).ToList());
		}
