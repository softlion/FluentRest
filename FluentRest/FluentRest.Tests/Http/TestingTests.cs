...
namespace FluentRest.Test.Http
{
	...
	public class TestingTests : HttpTestFixtureBase
	{
		...
		public async Task can_setup_multiple_responses() {
			...
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveCalled("http://www.api.com/"));
		...
		public async Task can_assert_verb() {
			...
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveMadeACall().WithVerb(HttpMethod.Get));
		}
		...
		public async Task can_assert_query_params() {
			...
			// failures
			Assert.ThrowsException<HttpTestException>(() =>
		...
		public async Task can_assert_basic_auth() {
			...
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveMadeACall().WithBasicAuth("me", "wrong"));
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveMadeACall().WithBasicAuth("you"));
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveMadeACall().WithBasicAuth("m*", "*out"));
			Assert.ThrowsException<HttpTestException>(() => HttpTest.ShouldHaveMadeACall().WithOAuthBearerToken());
		}
		...
		public async Task can_simulate_timeout() {
			...
			catch (FluentRestHttpTimeoutException ex) {
		...
		public async Task can_deserialize_default_response_more_than_once() {
			...
			Assert.IsNull(resp);
			// issue: couldn't deserialize here due to reading stream twice
		...
		public void can_configure_settings_for_test() {
			Assert.IsTrue(new FluentRestRequest().Settings.Redirects.Enabled);
			using (new HttpTest().Configure(settings => settings.Redirects.Enabled = false)) {
				Assert.IsFalse(new FluentRestRequest().Settings.Redirects.Enabled);
			}
