...
namespace FluentRest.Http.Testing
{
	...
	public class HttpCallAssertion
	{
		...
		private IList<FluentRestDetail> _calls;
		...
		public HttpCallAssertion(IEnumerable<FluentRestDetail> loggedCalls, bool negate = false) {
		...
		/// </summary>
		/// <param name="match">Predicate (usually a lambda expression) that tests a FluentRestDetail and returns a bool.</param>
		/// <param name="description">A description of what is being asserted.</param>
		public HttpCallAssertion With(Func<FluentRestDetail, bool> match, string? description = null) 
		{
		    if (!string.IsNullOrEmpty(description))
			    _expectedConditions.Add(description);
		...
		public HttpCallAssertion Without(Func<FluentRestDetail, bool> match, string? descrip = null) {
		...
		public HttpCallAssertion WithUrlPattern(string urlPattern) {
			return With(c => FluentRestDetailExtensions.MatchesUrlPattern(c.Request.Url, urlPattern), $"URL pattern {urlPattern}");
		}
		...
		public HttpCallAssertion WithRequestBody(string bodyPattern) {
			return With(c => FluentRestDetailExtensions.MatchesPattern(c.RequestBody, bodyPattern), $"body {bodyPattern}");
		}
		...
		public HttpCallAssertion WithRequestJson(object body) {
			var serializedBody = FluentRestHttp.GlobalSettings.JsonSerializer.Serialize(body);
		...
		public HttpCallAssertion WithRequestUrlEncoded(object body) {
			var serializedBody = FluentRestHttp.GlobalSettings.UrlEncodedSerializer.Serialize(body);
			return WithRequestBody(serializedBody);
		}
		#endregion
		...
		public HttpCallAssertion WithBasicAuth(string username = "*", string password = "*") =>
			With(call => 
			{
				...
				if (val == null) 
					return false;
				if (!val.OrdinalStartsWith("Basic ")) 
					return false;
				...
				try 
				{
					...
					return
						creds.Length == 2 &&
						FluentRestDetailExtensions.MatchesPattern(creds[0], username) &&
						FluentRestDetailExtensions.MatchesPattern(creds[1], password);
				}
