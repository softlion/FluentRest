...
namespace FluentRest.Http
{
	...
	public static class CookieCutter
	{
		...
		/// <summary>
		/// Parses a Set-Cookie response header to a FluentRestCookie.
		/// </summary>
		/// <param name="url">The URL that sent the response.</param>
		/// <param name="headerValue">Value of the Set-Cookie header.</param>
		/// <returns></returns>
		public static FluentRestCookie ParseResponseHeader(string url, string headerValue) {
			if (string.IsNullOrEmpty(headerValue)) return null;

			FluentRestCookie cookie = null;
			foreach (var pair in GetPairs(headerValue)) {
				if (cookie == null)
					cookie = new FluentRestCookie(pair.Name, pair.Value.Trim('"'), url, DateTimeOffset.UtcNow);
		...
		/// </summary>
		public static bool IsValid(this FluentRestCookie cookie, out string reason) {
		...
		/// </summary>
		public static bool IsExpired(this FluentRestCookie cookie, out string reason) {
		...
		/// </summary>
		public static bool ShouldSendTo(this FluentRestCookie cookie, Url requestUrl, out string reason) {
		...
		private static bool IsDomainMatch(this FluentRestCookie cookie, Url requestUrl, out string reason) {
			reason = "ok";
		...
		private static bool IsPathMatch(this FluentRestCookie cookie, Url requestUrl, out string reason) {
			reason = "ok";
