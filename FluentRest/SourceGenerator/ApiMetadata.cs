	...
	public static class ApiMetadata
	{
		...
		public static IEnumerable<ExtensionMethod> GetUrlReturningExtensions(MethodArg extendedArg) {
			ExtensionMethod Create(string name, string descrip) => new ExtensionMethod(name, descrip)
				.Extends(extendedArg)
				.Returns("Url", "A new FluentRest.Url object.");
		...
		/// <summary>
		/// Mirrors methods defined on IFluentRestRequest and IFluentRestClient. We'll auto-gen them for Url, Uri, and string. 
		/// </summary>
		public static IEnumerable<ExtensionMethod> GetRequestReturningExtensions(MethodArg extendedArg) {
			ExtensionMethod Create(string name, string descrip) => new ExtensionMethod(name, descrip)
				.Extends(extendedArg)
				.Returns("IFluentRestRequest", "A new IFluentRestRequest.");

			// header extensions
			yield return Create("WithHeader", "Creates a new FluentRestRequest and sets a request header.")
				.AddArg("name", "string", "The header name.")
				.AddArg("value", "object", "The header value.");
			yield return Create("WithHeaders", "Creates a new FluentRestRequest and sets request headers based on property names/values of the provided object, or keys/values if object is a dictionary, to be sent.")
			...
			yield return Create("WithBasicAuth", "Creates a new FluentRestRequest and sets the Authorization header according to Basic Authentication protocol.")
			...
			yield return Create("WithCookies", "Creates a new FluentRestRequest and adds name-value pairs to its Cookie header based on property names/values of the provided object, or keys/values if object is a dictionary. " +
			                                   "To automatically maintain a cookie \"session\", consider using a CookieJar or CookieSession instead.")
				.AddArg("values", "object", "Names/values of HTTP cookies to set. Typically an anonymous object or IDictionary.");
			yield return Create("WithCookies", "Creates a new FluentRestRequest and sets the CookieJar associated with this request, which will be updated with any Set-Cookie headers present in the response and is suitable for reuse in subsequent requests.")
				.AddArg("cookieJar", "CookieJar", "The CookieJar.");
			yield return Create("WithCookies", "Creates a new FluentRestRequest and associates it with a new CookieJar, which will be updated with any Set-Cookie headers present in the response and is suitable for reuse in subsequent requests.")
				.AddArg("cookieJar", "CookieJar", "The created CookieJar, which can be reused in subsequent requests.", isOut: true);

			// settings extensions
			yield return Create("ConfigureRequest", "Creates a new FluentRestRequest and allows changing its Settings inline.")
				.AddArg("action", "Action<FluentRestHttpSettings>", "A delegate defining the Settings changes.");
			yield return Create("WithTimeout", "Creates a new FluentRestRequest and sets the request timeout.")
			...
			yield return Create("AllowHttpStatus", "Creates a new FluentRestRequest and adds a pattern representing an HTTP status code or range of codes which (in addition to 2xx) will NOT result in a FluentRestHttpException being thrown.")
				.AddArg("pattern", "string", "Examples: \"3xx\", \"100,300,600\", \"100-299,6xx\"");
			yield return Create("AllowHttpStatus", "Creates a new FluentRestRequest and adds an HttpStatusCode which (in addition to 2xx) will NOT result in a FluentRestHttpException being thrown.")
				.AddArg("statusCodes", "params HttpStatusCode[]", "The HttpStatusCode(s) to allow.");
			yield return Create("AllowAnyHttpStatus", "Creates a new FluentRestRequest and configures it to allow any returned HTTP status without throwing a FluentRestHttpException.");
			yield return Create("WithAutoRedirect", "Creates a new FluentRestRequest and configures whether redirects are automatically followed.")
				.AddArg("enabled", "bool", "true if FluentRest should automatically send a new request to the redirect URL, false if it should not.");

			yield return Create("WithClient", "Creates a new FluentRestRequest and configures it to use the given IFluentRestClient.")
				.AddArg("client", "IFluentRestClient", "The IFluentRestClient to use to send the request.");

			// a couple oddballs that don't really belong here but here we are

			yield return new ExtensionMethod("DownloadFileAsync", "Creates a new FluentRestRequest and asynchronously downloads a file.")
			...
			yield return new ExtensionMethod("PostMultipartAsync", "Creates a FluentRestRequest and sends an asynchronous multipart/form-data POST request.")
				.Returns("Task<IFluentRestResponse>", "A Task whose result is the received IFluentRestResponse.")
				.Extends(extendedArg)
		...
		public static IEnumerable<HttpExtensionMethod> GetHttpCallingExtensions(MethodArg extendedArg) {
			return
				...
				where IsSupportedCombo(verb, reqType, respType, extendedArg.Type, isGeneric)
